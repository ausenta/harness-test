...
2. **Stay in scope**: touch only files listed in the item's `file-surface`. If the task genuinely requires touching another file, STOP and write the question into the item file under `## Questions for Orchestrator` — do not proceed. If a harness-level file OUTSIDE your surface (e.g. a CI workflow) blocks your gates, raise it in Questions AND, if the orchestrator fixes it on `main`, you may sync that fix **read-only** into your worktree to run green locally — but do NOT commit out-of-surface files. Leave them as working-tree changes; the board rebase carries the main-side fix. Never commit to a file that scored your work green.
